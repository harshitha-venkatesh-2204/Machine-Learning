...
The relationship between the y and the x seems to be slightly negative. Conversely, it seems that for positive input values, the response increases dramatically. It seems that there is a non-linear relationship between the input and the response because a single straight line cannot seem to effectively describe the relationship. But, there appears to be an increase in the y > 30 observation that distinguishes it from the other findings. If this information were ignored, it would seem that the reaction predominantly decreases as the input increases.However, The plot doesn't tell us if the relationship is linear or non-linear.
...
rmse_data <- data.frame(
  Model = paste("fit_lm_", 1:9),
  RMSE = c(
    modelr::rmse(fit_lm_1, data = df_low_high),
    modelr::rmse(fit_lm_2, data = df_low_high),
    modelr::rmse(fit_lm_3, data = df_low_high),
    modelr::rmse(fit_lm_4, data = df_low_high),
    modelr::rmse(fit_lm_5, data = df_low_high),
    modelr::rmse(fit_lm_6, data = df_low_high),
    modelr::rmse(fit_lm_7, data = df_low_high),
    modelr::rmse(fit_lm_8, data = df_low_high),
    modelr::rmse(fit_lm_9, data = df_low_high)
  )
)
...
```{r}
ggplot(rmse_data, aes(x = Model, y = RMSE, group = 1)) +
  geom_line(color = "skyblue", size = 1) +
  geom_point(color = "skyblue", size = 3) +
  geom_text(aes(label = sprintf("%.4f", RMSE)), vjust = -0.5, color = "black") +
  labs(title = "Training Set RMSE for Each Model", x = "Model", y = "RMSE") +
  theme_minimal()
...
```{r, solution_01d_a}
### add as many code chunks as necessary
ggplot(rsquare_data, aes(x = Model, y = RSQUARE, group = 1)) +
  geom_line(color = "lightpink", size = 1) +
  geom_point(color = "lightpink", size = 3) +
  geom_text(aes(label = sprintf("%.4f", RSQUARE)), vjust = -0.5, color = "black") +
  labs(title = "Training Set RSquare for Each Model", x = "Model", y = "RSQUARE") +
  theme_minimal()
```
### 1e)
...
The model with the lowest RMSE value is the best model in terms of RMSE. The model with the highest R-squared value is the best one, according to R-squared. The 9th degree polynomial in this application has the highest training set R-squared and the lowest training set RMSE.
...
The second-degree model shows a positive estimate for the quadratic feature, but the sixth and ninth-degree models do not deem it statistically significant. These variations draw attention to how models treat different attributes.

Significant uncertainty is highlighted in the 9th-degree polynomial's coefficient plot, which is especially evident for the 4th-degree feature. The uncertainty interval, which typically ranges from -40 to +20, shows a high degree of variability and imprecise coefficient estimations. In conclusion, the 9th-degree polynomial stands out for its heightened uncertainty, notably with regard to the 4th-degree feature, even though several coefficients exhibit divergence among models.
...
ggplot(df_viz_1, aes(x = x)) +
  geom_line(aes(y = pred_trend), color = "purple") +
  labs(title = "Linear Relationship Trend", x = "x", y = "Predicted Trend") +
  theme_minimal()
```
...
ggplot(df_viz_1, aes(x = x)) +
  ...
  geom_line(aes(y = pred_trend), size = 1, color = 'black') +
  labs(title = "Linear Relationship Trend and Original Training Set", x = "x", y = "y") +
  theme_minimal()
```
...
ggplot(df_viz_all, aes(x = x, y = pred_trend)) +
  geom_line(color = "black") +
  facet_wrap(~poly_degree, ncol = 3) +
  labs(title = "Predicted Trends for Different Polynomial Models", x = "x", y = "Predicted Trend") +
  theme_minimal()
```
...
It's possible that the model is "overfitting" the training set, identifying details that might not accurately reflect the underlying patterns. It is difficult to tell whether the anticipated trends—particularly those that fall outside of the training data range—have factual backing, which makes it difficult to assess whether the complexity is real or just a model artifact. This emphasizes how crucial it is to take into account both the model's behavior beyond observed values and its performance on training data.
...
ggplot() +
  geom_point(data = df_train, aes(x = x, y = y, color = "Training Set", shape = "Training Set"), size = 3) +
  geom_point(data = df_holdout, aes(x = x, y = y, color = "Hold-out Test Set", shape = "Hold-out Test Set"), size = 3) +
  scale_color_manual(values = c("Training Set" = "lightblue", "Hold-out Test Set" = "red")) +
  scale_shape_manual(values = c("Training Set" = 16, "Hold-out Test Set" = 17)) +
  labs(title = "Training/Test Data Split Visualization",
       x = "Input (x)",
       y = "Output (y)",
       color = "Set",
       shape = "Set") +
...
```{r, solution_03d_a}
### add as many code chunks as you feel are necessary
ggplot(rmse_results, aes(x = Model, y = RMSEDATA, group = 1)) +
  geom_line(color = "yellow", size = 1) +
  geom_point(color = "yellow", size = 3) +
  geom_text(aes(label = sprintf("%.4f", RMSEDATA)), vjust = -0.5, color = "black") +
  labs(title = "Hold-Out Set RMSE for Each Model", x = "Model", y = "RMSEDATA") +
  theme_minimal()
```
...
By looking at the row with the lowest RMSE value, we can determine which model is the best one based on the hold-out evaluation.In terms of holdout test set performance, the quadratic model (degree 2) performs the best. 
...
The 9th degree polynomial fit on the full original dataset (fit_lm_9) and the fit on the random training split (fit_train_9) show different signs for the 4th and 6th degree features. This is evident from a comparison of the coefficient summaries between the two models. 
...
The holdout test split indicated that the quadratic model was the most effective. All coefficients have the same sign and the uncertainty levels are constant, despite differences in the number of statistically significant features and the coefficient estimations.  In comparison to the 9th degree polynomial, the coefficients are fundamentally stable and so depend less on the training data.
...
The resampling scheme specified with method = "cv" and number = 5 indicates 5-fold cross-validation.  Therefore, an individual model will be trained and tested 5 times in total.
...
I chose "RMSE" (Root Mean Squared Error) as the primary performance metric.It provides a good indication of how well the model's predictions align with the observed data, giving equal weight to all errors
...
summary_resampling <- summary(lowhigh_results, metric = my_metric)
summary_resampling
```
...
dotplot_resampling <- dotplot(lowhigh_results, metric = my_metric)
dotplot_resampling
```
...
The model with the lowest RMSE is fit_03, the third-degree polynomial model. It has an RMSE of 5.068396, which is the smallest among all the models. Therefore, according to the RMSE metric, the third-degree polynomial model is considered the best among the evaluated models.
...
lm_model_3 <- mod_lowhigh_3$finalModel
lm_model_2 <- mod_lowhigh_2$finalModel
lm_model_1 <- mod_lowhigh_1$finalModel
...
The three models seem to have the same amount of uncertainty, and the quadratic and cubic models both indicate that they are confident non-linear interactions.
...
ggplot(df_low_low , aes(x = x, y = y)) +
  geom_point(size = 3) +
  labs(title = "Scatter Plot for Low Noise Level Data",
       x = "Input (x)",
       y = "Output (y)") +
  theme_minimal()
```
...
Both the scale and the scatter plot distribution in 1a and 5a are different.The differences between the two datasets (df_low_low and df_low_high) and the noise levels connected to each dataset could be the cause of these variations.
...
summary_low_noise <- summary(lowlow_results, metric = my_metric)
summary_low_noise
```
...
dotplot_resampling <- dotplot(lowlow_results, metric = my_metric)
dotplot_resampling
```

Which model is the best?  

The model with the lowest RMSE is fit_06, so it can be considered the best model based on the RMSE metric
...
The 6th degree polynomial was found to be the best fit for the low noise level data by the resampling approach.  The 6th degree polynomial was considered less effective than simpler models on the high noise level data. 
...
Both the scale and the scatter plot distribution in 1a and 6a are different.The differences between the two datasets (df_high_high and df_low_high) and the noise levels connected to each dataset could be the cause of these variations.
...
```{r, solution_06c_a}
### print the resampling summary for the performance metric
summary_low_noise <- summary(highhigh_results, metric = my_metric)
summary_low_noise
```
...
```{r, solution_06c_b}
### visualize the dotplot for the performance metric
dotplot_resampling <- dotplot(highhigh_results, metric = my_metric)
dotplot_resampling
```
...
More complex models—specifically, the quadratic model—performed better for bigger sample sizes, while the simpler cubic model was thought to be optimal for smaller samples.
...
Sample size and noise level have an impact on recognizing complicated behavior since noisy data makes it harder to tell if complex patterns are real or just random. Simplier models may be preferred because they are less impacted by noise, especially in situations when sample numbers are small and noise levels are high. More data are available from larger sample sizes, which also increase the confidence in seeing intricate patterns within the noise.
