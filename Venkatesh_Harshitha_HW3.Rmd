...
```{r, solution_01b}
### your code here
eventFraction <- mean(model_pred_df$obs_class == "event")
eventFraction
```

Is the data set balanced?  
Yes, the resulting value is close to 0.5, so it is a balanced data set.
...
model_pred_df %>% 
  ...
  labs(title = "Overlay Plot of Predicted Probability and Binary Response",
       x = "Input (x)",
...
The plot shows that the observed binary responses match the expected probability of the model. Few y == 0 observations are found in regions with higher predicted probabilities (>75%), which is in line with expectations. The visual agreement between predictions and observations is supported by the fact that there are still y == 1 events even at the lowest predicted probability (about 22%).
...
### your code here
model_class_0.5 %>% 
summarize(mean(pred_class == obs_class))
...
True-Negatives (TN) are represented by the count for "non_event" in the combination "non_event" vs. "non_event" and the count for "event" in the combination "event" vs. "non_event" in the output you gave.

Thus, in the provided output:
6 observations are false-positives (FP).
53 observations are True Negatives (TN).
...
tpr_model_class_0.5 <- tp / (tp + fn)
tpr_model_class_0.5
fpr_model_class_0.5 <- fp / (fp + tn)
fpr_model_class_0.5
```
...
tpr_model_class_0.7 <- tp / (tp + fn)
tpr_model_class_0.7
fpr_model_class_0.7 <- fp / (fp + tn)
fpr_model_class_0.7
```
...
tpr_model_class_0.3 <- tp / (tp + fn)
tpr_model_class_0.3
fpr_model_class_0.3 <- fp / (fp + tn)
fpr_model_class_0.3
```
...
threshold_values <- c(0.3, 0.5, 0.7)
TPR_values <- c(tpr_model_class_0.3, tpr_model_class_0.5, tpr_model_class_0.7)
FPR_values <- c(fpr_model_class_0.3, fpr_model_class_0.5, fpr_model_class_0.7)
...
### model 2
set.seed(2021)
mod_2_acc <- train(y ~ x1 + I(x1^2) + x2 + I(x2^2) + x3 + I(x3^2) + x4 + I(x4^2),
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
### model 3
set.seed(2021)
mod_3_acc <- train(y ~ x1 + I(x1^2) + x2 + I(x2^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
### model 4
set.seed(2021)
mod_4_acc <- train(y ~ x1 + I(x1^2) + x3 + I(x3^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
### model 5
set.seed(2021)
mod_5_acc <- train(y ~ x1 + I(x1^2) + x4 + I(x4^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
### model 6
set.seed(2021)
mod_6_acc <- train(y ~ x2 + I(x2^2) + x3 + I(x3^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
### model 7
set.seed(2021)
mod_7_acc <- train(y ~ x2 + I(x2^2) + x4 + I(x4^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
### model 8
set.seed(2021)
mod_8_acc <- train(y ~ x3 + I(x3^2) + x4 + I(x4^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_acc,
                   preProcess = c("center", "scale"),
...
```{r, solution_04e_02}
### add your code here
summary_results <- summary(acc_results, metric = 'Accuracy')
summary_results
```
...
```{r, solution_04e_03}
### add your code here
dotplot(acc_results, metric = 'Accuracy', col = "blue", main = "Resample-Averaged Accuracy per Model")
...
Mod_3 has the highest mean accuracy of all the models, at 0.8079, based on the measure "Accuracy" and the summary of resamples. But one metric shouldn't be used to determine which model is the best.
...
```{r, solution_04fa1, eval=TRUE}
confusionMatrix(mod_3_acc) 
```

```{r, solution_04fa2, eval=TRUE}
confusionMatrix(mod_2_acc) 
```

```{r, solution_04fa3, eval=TRUE}
confusionMatrix(mod_5_acc) 
```

```{r, solution_04fa, eval=TRUE}
confusionMatrix(mod_8_acc) 
```
The second-best model (mod_2) tends to have a somewhat higher False-Positive rate, whereas the best model (mod_3) is more conservative in its event prediction. The False-Negative rate of the worst model (mod_8) is higher than that of the second-worst model (mod_5). Depending on the particular objectives and specifications of the application, the False-Positive and False-Negative behavior offers insights into the advantages and disadvantages of each model, assisting in the process of making an informed choice.
...
### model 2
set.seed(2021)
mod_2_roc <- train(y ~ x1 + I(x1^2) + x2 + I(x2^2) + x3 + I(x3^2) + x4 + I(x4^2), 
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
### model 3
set.seed(2021)
mod_3_roc <- train(y ~ x1 + I(x1^2) + x2 + I(x2^2),
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
### model 4
set.seed(2021)
mod_4_roc <- train(y ~ x1 + I(x1^2) + x3 + I(x3^2),
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
### model 5
set.seed(2021)
mod_5_roc <- train(y ~ x1 + I(x1^2) + x4 + I(x4^2),
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
### model 6
set.seed(2021)
mod_6_roc <- train(y ~ x2 + I(x2^2) + x3 + I(x3^2),
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
### model 7
set.seed(2021)
mod_7_roc <- train(y ~ x2 + I(x2^2) + x4 + I(x4^2),
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
### model 8
set.seed(2021)
mod_8_roc <- train(y ~ x3 + I(x3^2) + x4 + I(x4^2),
                   data = df,
                   method = 'glm',
                   metric = metric_roc,
                   preProcess = c("center", "scale"),
...
Mod_3 appears to have the highest mean accuracy of all the models, at roughly 0.8079167, based on the measure "Accuracy" and the summary of resamples. But one metric shouldn't be used to determine which model is the best.
...
dotplot(roc_results)
```
At the default threshold, mod_4 has the lowest False-Positive Rate (1 - Specificity) while mod_3 has the highest True-Positive Rate (Sensitivity).
...
With a ROC curve that is closest to the top-left corner of the plot, the best model mod 3 has high sensitivity and specificity. A model that is not working well would have a ROC curve that is near the 45-degree line, or diagonal, which is mod 8 from the plot. 
...
The pred_test_1 dataframe's event and non_event columns, which record the probability of the event and the probability of the non-event, respectively, indicate the expected probabilities for a binary outcome variable. Given that the result is binary, the total of these predicted probability for every observation should equal 1.
...
test_df_1_b <- test_df_1 %>%
  mutate(pred_bin = cut(pred_test_1$event, breaks = seq(0, 1, length.out = 11)))
```
...
```{r, solution_06e}
### your code here
ggplot(test_df_1_b, aes(x = pred_bin)) +
  geom_bar() +
...
When creating a stacked bar chart to display the distribution of observed findings within predicted probability bins, bins have a constant color. Bins that are blue in color are used to depict instances in which all observations are events.Bins that are red in color indicate that all observations are non-events.
A visual inspection of the stacked bar chart can reveal the predicted probability bins' structure and which data subsets the model consistently links to a particular outcome.
...
test_df_1_b %>% 
  ...
  labs(title = "Stacked Bar Chart of Observed Outcome per Bin",
       x = "Predicted Probability Bins",
       y = "Count") +
...
test_df_1_b %>% 
  ...
  geom_bar(mapping = aes(fill = y),
           position = 'fill') +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Proportion of Observed Outcome per Bin",
       x = "Predicted Probability Bins",
       y = "Proportion") +
  theme_bw()
...
my_calcurve_1 <- test_df_1_b %>% 
  ...
  summarize(
    num_obs = n(),
    num_events = sum(y == 'event'),
    prop_event = mean(y == 'event'),
    min_prob = min(event),
    max_prob = max(event)
  ) %>% 
  mutate(mid_bin = (min_prob + max_prob) / 2,
         .groups = 'drop') %>%
  select(pred_bin, prop_event,mid_bin)
...
```{r, solution_07b}
### your code here
calibration_curve <- my_calcurve_1 %>% 
  ggplot(aes(x = mid_bin, y = prop_event)) +
  geom_line(color='coral',size = 1) +
  geom_point(color='coral',size = 3) +
  geom_abline(slope = 1, intercept = 0, color = 'grey', linetype = 'dashed') +
  coord_equal(xlim = c(0, 1), ylim = c(0, 1)) +
  labs(title = "Calibration Curve",
       x = "Predicted Probability Bins (Midpoints)",
       y = "Empirical Proportion of Events")
...
test_df_3_b <- test_df_3 %>%
  mutate(pred_bin = cut(event, breaks = seq(0, 1, by = 0.1))) 
...
ggplot(test_df_3_b, aes(x = pred_bin)) +
  ...
  labs(title = "Number of Observations per Bin (Model 3)",
       x = "Predicted Probability Bins",
...
```{r, solution_08c}
### your code here
calibration_curve <- my_calcurve_3 %>% 
  ggplot(aes(x = mid_bin, y = prop_event)) +
...
```{r, solution_08d}
### your code here
calibration_curve <- my_calcurve_8 %>% 
  ggplot(aes(x = mid_bin, y = prop_event)) +
...
The three models' performance is shown in significant detail by the calibration curves, and Model 3 stands out as the best-calibrated model because of its constant alignment with the ideal curve. This suggests that the observed event proportions and predicted probability have a consistent connection. Model 3 exhibits adaptability by capturing a wider spectrum of event probabilities, whereas Model 8 fails to forecast a diverse range of probabilities, remaining close to a fixed 50% likelihood.
...
There is a consistent pattern in the calibration curves obtained manually and by the xyplot() picture generated for the earlier model. Between the two methods, the observed behavior—where empirical event proportions rise above model 1 forecasts in the first bins and decrease in the following bins—remains consistent.

#### SOLUTION

```{r, solution_09a}
### your code here
cal_curve_model_1 <- test_df_1 %>%
  calibration(y ~ event, data = ., cuts = 10) %>%
  xyplot()
cal_curve_model_1
...
There is a consistent pattern in the calibration curves obtained manually and by the xyplot() picture generated for the earlier model. Model 3 demonstrated the most reliable alignment with the 45-degree line, according to the comparison, suggesting better calibration. Consistency, the range of recorded event probabilities, and overall calibration quality were among the factors taken into account in the evaluation.
...
There is a consistent pattern in the calibration curves obtained manually and by the xyplot() picture generated for the earlier model.Model 8's calibration curve covers all bins, however its irrelevant predictions may cause interpretations to be erroneous. Manual inspection made it clear that the model cannot forecast probabilities lower than 30 percent, highlighting the need for close inspection when assessing models.
