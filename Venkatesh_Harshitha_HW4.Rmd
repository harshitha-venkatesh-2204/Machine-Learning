...
The Bernoulli distribution's mean and the event probability's MLE are the same.
...
Given the event probability μ, the Bernoulli likelihood of the single observation, 

$$ 
p(x_n \mid \mu) = \text{Bernoulli}(x_n \mid \mu) = \mu^{x_n} (1 - \mu)^{1 - x_n}
$$
Natural log on both sides,

$$ 
\log(p(x_n \mid \mu)) = \log(\mu^{x_n} (1 - \mu)^{1 - x_n})
$$
Simplify,


$$ 
\log(\mu^{x_n}(1-\mu)^{1-x_n}) = \log(\mu^{x_n}) + \log((1-\mu)^{1-x_n})
$$


$$ 
\log(\mu^{x_n}) + \log((1-\mu)^{1-x_n}) = x_n \log(\mu) + (1-x_n) \log(1-\mu)
$$

the Bernoulli likelihood is,

$$ 
\log(\text{Bernoulli}(x_n \mid \mu)) = x_n \log(\mu) + (1 - x_n) \log(1 - \mu)
$$
...
{
   log_pmf <- xobs * log(prob) + (1 - xobs) * log(1 - prob)
   return(log_pmf)
}
...
result_1 <- log_bernoulli_pmf(x[1], 0.25)
result_1

result_2 <- log_bernoulli_pmf(x[2], 0.25)
result_2

result_3 <- log_bernoulli_pmf(x[3], 0.25)
result_3

result_4 <- log_bernoulli_pmf(x[4], 0.25)
result_4

result_5 <- log_bernoulli_pmf(x[5], 0.25)
result_5
...
The values are the same as the responses to the previous question because the function was previously called once for each observation rather than all at once.

```{r, solution_1g}

result_multiple <- log_bernoulli_pmf(x[1:5], 0.25)
result_multiple

length(result_multiple)
...
The "complete" log-likelihood is obtained by adding up the log-likelihoods of each observation

$$ 
\log(p(x \mid \mu)) = \sum_{n=1}^{N} \log(\text{Bernoulli}(x_n \mid \mu))
$$

Substitute for n,

$$ 
\log(p(x \mid \mu)) = \sum_{n=1}^{N} (x_n \log(\mu) + (1 - x_n) \log(1 - \mu))
$$
$$ 
\log(p(x \mid \mu)) = \sum_{n=1}^{N} (x_n \log(\mu)) + \sum_{n=1}^{N} ((1 - x_n) \log(1 - \mu))
$$
$$ 
\log(p(x \mid \mu)) = \log(\mu) \sum_{n=1}^{N} x_n + \log(1-\mu) \sum_{n=1}^{N} (1 - x_n)
$$
Summation of Binary Events

$$ 
\sum_{n=1}^{N} x_n = H
$$
Summation of Complementary Binary Events

$$ 
\sum_{n=1}^{N} (1 - x_n) = AB - H
$$
Log-Likelihood Expression with Counts:

$$ 
\log(p(x \mid \mu)) = \log(\mu) \times H + \log(1-\mu) \times (AB - H)
$$
...
complete_log_likelihood <- sum(log_bernoulli_pmf(x, 0.25))
...
plot_data <- data.frame(mu = mu_grid, log_likelihood = log_lik_xa)

ggplot(plot_data, aes(x = mu, y = log_likelihood)) +
  geom_line() +
  labs(title = "Log-Likelihood vs. Event Probability",
       x = "Event Probability (mu)",
       y = "Log-Likelihood")
...
The most likely event probability given the data is indicated by the Maximum Likelihood Estimate (MLE), which is found at the apex of the log-likelihood curve. The curve's flatness at the MLE, which indicates a zero slope, draws attention to the parameter's best fit and paves the way for more complex optimization ideas in statistical modeling.
...
$$
Binomial Probability Mass Function for Number of Hits

$$ 
p(H \mid AB, \mu) = \binom{AB}{H} \mu^H (1 - \mu)^{AB - H}
$$
Log-Likelihood for Binomial Distribution
$$ 
\log[p(H \mid AB, \mu)] = \log[\binom{AB}{H}] + \log[\mu^H] + \log[(1 - \mu)^{AB - H}]
$$
Proportional Log-Likelihood for Binomial Distribution,
$$ 
\log[p(H \mid AB, \mu)] \propto \log[\mu^H] + \log[(1 - \mu)^{AB - H}]
$$
$$ 
\log[p(H\mid AB,\mu)] \propto H \times \log[\mu] + (AB - H) \times \log[1-\mu]
$$
...
tibble::tibble(
  ...
  geom_line(size = 1.1) +
  geom_vline(xintercept = player_hits / player_atbats,
             color = 'red', linetype = 'dashed') +
  labs(title = "Log-Likelihood vs. Event Probability (Binomial Formulation)",
       x = "Event Probability (mu)",
       y = "Log-Likelihood")
...
The Binomial formulation's MLE is the same as the MLE linked to the Bernoulli formulation.
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(player_hits, size = player_atbats, prob = mu, log = TRUE)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) +
  geom_line(size = 1.1) +
  geom_vline(xintercept = player_hits / player_atbats,
...
They are the same. The MLE correlates to the same μ value, and the forms are the same. The values on the y-axis are the only thing that differ.
...
tibble::tibble(
  ...
  geom_line(size = 1.1) +
  labs(title = "Uniform Prior Distribution on Event Probability (μ)",
       x = "Event Probability (μ)",
...
tibble::tibble(
  ...
  ggplot(aes(x = mu, y = beta_pdf)) +
  geom_line(size = 1.1) +
  labs(title = "Informative Prior Distribution on Event Probability (μ)",
       x = "Event Probability (μ)",
       y = "Probability Density") +
  theme_minimal()
...
The uniform prior denotes the highest level of uncertainty and gives equal weight to all possible values of the event probability. It has a flat distribution. Alternatively, the informative prior centers its density around 0.25, indicating the assumption that a player in the major leagues will receive a few hits, but is unlikely to hit for home run more than 50% of the time. The prior selection process reveals how well Bayesian analysis balances previous knowledge with objectivity.
...
$$
Un-Normalized Log-Likelihood for Binomial Distribution
$$ 
\log(p(m \mid N, \mu)) \propto m \cdot \log(\mu) + (N - m) \cdot \log(1 - \mu) 
$$
Binomial Probability Mass Function for Number of Events,
$$ 
p(m \mid N, \mu) = \binom{N}{m} \mu^m (1 - \mu)^{N - m}
$$
Log-Transformed Binomial Probability Mass Function for Number of Events
$$ 
\log[p(m \mid N,\mu)] = \log\left[\binom{N}{m}\right] + \log[\mu^m] + \log\left[(1-\mu)^{N-m}\right]
$$
Proportional Log-Transformed Binomial Probability Mass Function for Number of Events
...
$$ 
p(\mu \mid a, b) \propto \mu^{a-1} (1 - \mu)^{b-1}  
$$
Proportional Log-Density of the Beta Distribution for Unknown Probability μ
$$ 
\log(p(\mu \mid a, b)) \propto (a-1) \cdot \log[\mu] + (b-1) \cdot \log[1-\mu]
$$
...
$$ 
m \cdot \log(\mu) + (N - m) \cdot \log(1 - \mu) + (a - 1) \cdot \log(\mu) + (b - 1) \cdot \log(1 - \mu)
$$

Group like terms,

$$ 
(m + a - 1) \cdot \log(\mu) + (N - m + b - 1) \cdot \log(1 - \mu)
$$
(m+a−1) is the coefficient acting on log(μ). As a result, the revised or posterior new parameter needs to be:
$$ 
a_{{new}} = a + m
$$
(N−m+b−1) is the coefficient acting on log(1−μ). As a result, the posterior or updated bnew parameter needs to be:
$$ 
 b_{{new}} = b + (N - m)
$$
Partial Log-Posterior,
$$ 
(a_{\text{new}} - 1) \cdot \log(\mu) + (b_{\text{new}} - 1) \cdot \log(1 - \mu)  
$$
...
Log-Posterior Probability Density Function for Beta Distribution,
$$ 
\log(p(\mu \mid m, N)) \propto (a_{{new}} - 1) \cdot \log(\mu) + (b_{{new}} - 1) \cdot \log(1 - \mu)
$$
derivative of the log-posterior with respect to μ,

$$ 
\frac{d}{d\mu} \left( \log(p(\mu|m, N)) \right) = \frac{d}{d\mu} ( (a_{\text{new}} - 1) \log(\mu)) +   \frac{d}{d\mu} (b_{\text{new}} - 1) \log(1 - \mu))
$$
Simplify,

$$ 
(a_{\text{new}} - 1) \frac{d}{d\mu} \left( \log(\mu) \right) + (b_{\text{new}} - 1) \frac{d}{d\mu} \left( \log(1 - \mu) \right)
$$

derivative of log(μ) and log(1−μ) with respect to μ. 

$$ 
\frac{d}{d\mu}(\log(\mu)) = \frac{1}{\mu}
$$

$$ 
\frac{d}{d\mu}(log(1−μ))=- \frac{1}{1-\mu}
$$
Substitute both derivatives,

$$ 
\frac{d}{d\mu} \left( \log(p(\mu|m, N)) \right) = \frac{a_{\text{new}} - 1}{\mu} - \frac{b_{\text{new}} - 1}{1 - \mu}
$$
...
Solve for μMAP, set derivative of the log-posterior with respect to μ equal to zero

$$ 
\mu_{MAP} = \frac{d}{d\mu}(\log(p(\mu | m, N))) = \frac{a_{new}-1} \mu = \frac{b_{new}-1} \mu = 0
$$
solve for μ,

$$ 
\frac{a_{new}-1} \mu = \frac{b_{new}-1} \mu
$$
Cross-Multiply,

$$ 
({a_{new}-1}) (1-\mu) = ({b_{new}-1}) \mu
$$
$$ 
({a_{new}-1}) - ({a_{new}-1})\mu = ({b_{new}-1}) \mu
$$

Group μ together,
$$ 
({a_{new}-1}) = ({b_{new}-1}+{a_{new}-1})\mu = ({b_{new}}+{b_{new}-2})\mu
$$
...
posterior <- tibble::tibble(
  ...
  prior_type = c("uniform", "inform")
) %>% 
  mutate(m = player_hits,
         N = player_atbats) %>% 
  mutate(a_new = a + m,
         b_new = b + (N - m))
posterior 
...
posterior <- posterior %>%
  mutate(posterior_mean = a_new / (a_new + b_new),
         posterior_mode = (a_new - 1) / (a_new + b_new - 2),
         posterior_5th = qbeta(0.05, a_new, b_new),
         posterior_95th = qbeta(0.95, a_new, b_new))

posterior %>%
  select(prior_type, posterior_mean, posterior_mode, posterior_5th, posterior_95th)
...
posterior_uniform <- posterior %>% filter(prior_type == "uniform")

tibble::tibble(
  mu = mu_grid
) %>% 
  mutate(beta_pdf = dbeta(mu, posterior_uniform$a_new, posterior_uniform$b_new)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line(size = 1.1) +
  ggtitle("Posterior Beta Distribution - Uniform Prior") +
  xlab("Event Probability (μ)") +
...
posterior_informative <- posterior %>% filter(prior_type == "inform")

tibble::tibble(
  mu = mu_grid
) %>% 
  mutate(beta_pdf = dbeta(mu, posterior_informative$a_new, posterior_informative$b_new)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line(size = 1.1) +
  ggtitle("Posterior Beta Distribution - Informative Prior") +
  xlab("Event Probability (μ)") +
  ylab("Probability Density") +
  theme_bw()
...
While the informative prior shapes the posterior to coincide with the previous belief, producing a more limited and guided estimate of the event probability, the uniform prior produces a more dispersed and data-driven posterior.
...
```{r, solutioN_07a}
season_outcome <- tibble::tibble(
  a = c(a_uniform, a_inform),
  b = c(b_uniform, b_inform),
  prior_type = c("uniform", "inform")
) %>% 
  mutate(m = season_hits,
         N = season_atbats) %>% 
  mutate(a_new = a + m,
         b_new = b + (N - m))
season_outcome
...
season_outcome <- season_outcome %>% 
  mutate(
    post_mean = a_new / (a_new + b_new),
    post_mode = (a_new - 1) / (a_new + b_new - 2),
    post_q05 = qbeta(0.05, a_new, b_new),
    post_q95 = qbeta(0.95, a_new, b_new)
  ) %>% 
  select(prior_type, post_mean, post_mode, post_q05, post_q95)
...
tibble::tibble(
  ...
  mutate(
    beta_pdf = dbeta(mu, shape1 = 63, shape2 = 223)
  ) %>% 
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line(size = 1.1) +
  labs(title = "Posterior Beta Distribution (Informative Prior)",
       x = "Event Probability (μ)",
       y = "Density") +
  theme_bw()
...
tibble::tibble(
  ...
  mutate(
    beta_pdf = dbeta(mu, shape1 = 72, shape2 = 252)
  ) %>% 
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line(size = 1.1) +
  labs(title = "Posterior Beta Distribution (Informative Prior)",
...
Two different priors were taken into consideration while examining the impact of priors on the posterior distribution: an informative prior that centered around 0.25 and prevented extreme values, and a uniform prior with no prior bias. At small sample sizes, the Uniform prior showed noise or random variation sensitivity, but the Informative prior served as a barrier, prohibiting high probability values. Crucially, the prior's influence was greater in small sample sizes but lessened in big sample sizes. This discovery highlights the idea that the prior's influence can be overpowered by huge sample sizes, which reduces its significance. Although significant, the informative priors did not ensure the same outcomes for small and large sample sizes; rather, they were more useful as tools for determining the proper intervals that would yield conclusive responses.
